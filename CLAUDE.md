...
- **パスワード保護・暗号化された PDF を開くこと。** `QPdfWriter` は暗号化 PDF を生成できないため、この経路を検証する自動テスト用フィクスチャを作れない。テストも人間の実行証跡もゼロのまま実装だけが存在する状態は、機能ではなく負債と判断して落とした（開発者判断、2026-08-02）。パスワード保護・暗号化された PDF を渡した場合は、その旨を明示するメッセージで失敗として記録し、バッチは次のファイルへ進む（「エラー処理」節を参照）。
...
├── AGENTS.md
├── docs/
│   ├── qt-conventions.md    # 実装規約
│   ├── build.md             # ビルド・テスト・配布物
│   ├── usage.md             # 使い方と仕様（エラー文言の一覧を含む）
│   ├── known-issues.md      # 既知の問題・意図的な非対応・未検証項目
│   └── progress.md          # 進捗ログと決定事項
...
| PDF として不正 | 同上 |
| パスワード保護・暗号化 | 非対応。「パスワード保護・暗号化された PDF には対応していません」という理由でそのファイルを失敗として記録し、次のファイルへ進む（再入力プロンプトは出さない。非スコープ節を参照） |
...
2. `converter` は実行時に `QPdfWriter` で合成した小さな PDF（2〜3 ページ、1 ページは意図的に壊す）に対して統合テストを書く。出力 PNG の**サイズとページ数だけでなく、画素の性質も検証する**。
   - ピクセル完全一致のゴールデン比較はしない（Qt のバージョン差で壊れるため）。代わりに、バージョン差に影響されない不変な性質を見る。**透けている画素が 1 つもないこと、余白が紙の白であること**は必ず検証する。
   - **サイズとページ数しか見ていなかった結果、出力 PNG が「白い紙に黒い文字」ではなく「透明な背景に黒い文字」になっている不具合を、全テストが緑のまま通した（2026-08-04）。** 人間がダークモードのビューアで開くまで誰も気づかなかった。出力物そのものを見ないテストは、出力物を検証していない。
