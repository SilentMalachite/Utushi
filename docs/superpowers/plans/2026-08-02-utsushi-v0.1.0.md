...
注意: このファイル自身は `tests/` にあるため `magic72OnlyInRenderSize`・`noExceptionsNoProcessEvents` の走査対象外だが、`noQt5Api` は `tests/` も走査する。パターン文字列は生の禁止語を含まないよう正規表現の区切り（`\b` 等）を挟んである。変更時は自己マッチに注意。
...
> **注意（qt-conventions.md ライセンス節）:** この節に関わる内容は AI の判断で変えない。表示文言・アプリのライセンス（Apache-2.0 想定）は実装後に開発者の確認を得ること。

**Files:**
- Create: `app/license_dialog.hpp`, `app/license_dialog.cpp`, `LICENSE`（Apache-2.0 全文）
...
- [ ] **Step 1: `LICENSE` に Apache-2.0 全文を置く**（https://www.apache.org/licenses/LICENSE-2.0.txt の全文をそのまま保存）
...
LicenseDialog::LicenseDialog(QWidget* parent) : QDialog(parent) {
    ...
    text->setPlainText(tr(
        "Utsushi 0.1.0\n"
        "本アプリは Apache License 2.0 で配布されます（同梱の LICENSE を参照）。\n"
        "\n"
...
表示文言とライセンス選定（Apache-2.0）を開発者に確認してからコミットする。
...
jobs:
  build-test:
    ...
    runs-on: ${{ matrix.os }}
    env:
      QT_QPA_PLATFORM: offscreen   # tst_converter が QGuiApplication を要求するため
    steps:
